    ...
    for attempt in range(max_retries + 1):
        try:
            if "gemini" in model.lower():
                # Instanciar dinamicamente capturar a chave injetada pela request atual
                g_key = api_keys.get("GOOGLE_API_KEY") if api_keys else os.environ.get("GEMINI_API_KEY", "disabled")
                ...
                o_key = api_keys.get("OPENAI_API_KEY") if api_keys else os.environ.get("OPENAI_API_KEY", "disabled")
                async_client_openai = AsyncOpenAI(api_key=o_key)
