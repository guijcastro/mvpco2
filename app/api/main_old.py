...
# ─── Routers (incluídos conforme as fases avançam) ──────────────────────────
# Fase 1: routers esqueleto (serão implementados nas fases seguintes)
# from api.routers import transcribe, analyze, chat, reports
# app.include_router(transcribe.router, prefix="/api")
# app.include_router(analyze.router, prefix="/api")
# app.include_router(chat.router, prefix="/api")
# app.include_router(reports.router, prefix="/api")

# Placeholder para Fase 1 — confirma que o servidor está rodando
@app.get("/api/status", tags=["system"])
async def status():
    return {
        "phase": "Fase 1 — Fundação",
        "message": "API operacional. Routers de análise serão adicionados na Fase 2.",
        "docs": "/docs",
    }
